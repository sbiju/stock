...
class LoginView(FormView):
    ...
    stripe.api_key = "sk_test_9xZd8FGLMGEooYcCYWJtVpLk"
    ...
    def post_login(self, form):
      ...
      if self.request.user.first_name:
        id_do_usuario = self.request.user.first_name
        user = User.objects.get(username=self.request.user.username)
        username = self.request.user.username
        group = Group.objects.get(name='basico')
		
        if stripe.Customer.retrieve(id_do_usuario):
         customer = stripe.Customer.retrieve(id_do_usuario)
         if customer.subscriptions.data:
          status_sub = customer.subscriptions.data[0].status
          if ((status_sub == 'active') | (status_sub == 'trialing')):
           if user.username[-1:] == 'a':
